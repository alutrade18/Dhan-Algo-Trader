...
export default function RiskManager() {
  ...
  return (
    <div className="w-full space-y-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-background border border-border rounded-2xl p-6 w-80 space-y-4 shadow-2xl">
            ...
            <p className="text-xs text-muted-foreground">
              Enter your 4-digit PIN to {pinDialogFor === "ACTIVATE" ? "activate" : "deactivate"} the kill switch.
            </p>
            <Input type="password" placeholder="••••" maxLength={4} value={pinVerifyInput} onChange={e => setPinVerifyInput(e.target.value)} onKeyDown={e => e.key === "Enter" && void verifyPinAndProceed()} className="text-center text-xl tracking-widest font-mono" autoFocus />
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-background border border-border rounded-2xl p-6 w-80 shadow-2xl space-y-4">
            <div className="flex items-center gap-2">
              <Trash2 className="w-5 h-5 text-destructive" />
              <h3 className="font-semibold">Delete Kill Switch PIN</h3>
            </div>

            {deletePinStep === "enter" && (
              <>
                <p className="text-xs text-muted-foreground">Enter your current 4-digit PIN to proceed.</p>
                <div className="relative">
                  <Input
                    type={showDeleteFirst ? "text" : "password"}
                    placeholder="••••"
                    maxLength={4}
                    className="text-center text-xl tracking-widest font-mono pr-10"
                    value={deletePinFirst}
                    onChange={e => setDeletePinFirst(e.target.value.replace(/\D/g, "").slice(0, 4))}
                    autoFocus
                    onKeyDown={e => { if (e.key === "Enter" && deletePinFirst.length === 4) setDeletePinStep("confirm"); }}
                  />
                  <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground" onClick={() => setShowDeleteFirst(v => !v)}>
                    {showDeleteFirst ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="flex-1" onClick={closeDeleteModal}>Cancel</Button>
                  <Button size="sm" variant="destructive" className="flex-1" disabled={deletePinFirst.length !== 4} onClick={() => setDeletePinStep("confirm")}>
                    Next
                  </Button>
                </div>
              </>
            ...
            {deletePinStep === "confirm" && (
              <>
                <div className="flex items-center gap-2 rounded-lg bg-destructive/10 border border-destructive/20 px-3 py-2 text-xs text-destructive">
                  <AlertTriangle className="w-3.5 h-3.5 shrink-0" />Enter the same PIN again to confirm deletion.
                </div>
                <div className="relative">
                  <Input
                    type={showDeleteSecond ? "text" : "password"}
                    placeholder="••••"
                    maxLength={4}
                    className={`text-center text-xl tracking-widest font-mono pr-10 ${deletePinSecond.length === 4 && deletePinSecond !== deletePinFirst ? "border-destructive" : ""}`}
                    value={deletePinSecond}
                    onChange={e => setDeletePinSecond(e.target.value.replace(/\D/g, "").slice(0, 4))}
                    autoFocus
                    onKeyDown={e => { if (e.key === "Enter" && deletePinSecond === deletePinFirst) void handleConfirmDelete(); }}
                  />
                  <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground" onClick={() => setShowDeleteSecond(v => !v)}>
                    {showDeleteSecond ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
                {deletePinSecond.length === 4 && deletePinSecond !== deletePinFirst && (
                  <p className="text-[11px] text-destructive">PINs do not match</p>
                )}
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="flex-1" onClick={() => { setDeletePinStep("enter"); setDeletePinSecond(""); setShowDeleteSecond(false); }}>Back</Button>
                  <Button
                    size="sm"
                    variant="destructive"
                  ...
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />{genericSaveMutation.isPending ? "Deleting…" : "Confirm Delete"}
                  </Button>
                </div>
              </>
            )}
      ...
      <div className="grid grid-cols-3 gap-4 items-start">
        ...
        <Card className={pnlActive ? "border-primary/40" : ""}>
          <CardHeader
            ...
            badge={
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono text-muted-foreground bg-muted/30 border border-border/50 px-2 py-0.5 rounded-full">INTRADAY</span>
                {pnlActive && <Badge className="text-[10px] h-5 bg-primary/20 text-primary border border-primary/30 shadow-none">ACTIVE</Badge>}
              </div>
            }
          />
        ...
        <Card>
          ...
          <div className="px-5 pt-3 pb-5 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input type="time" value={autoSquareOffTime} onChange={e => setAutoSquareOffTime(e.target.value)} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm font-mono focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
            </div>
            ...
            {!isConnected ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 py-3 px-4 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm font-medium">
                  <Power className="w-4 h-4 shrink-0" />All order placement is currently blocked.
                </div>
                <Button
                  variant="outline"
                  size="sm"
              ...
              <div className="space-y-3">
                ...
                <Button variant="destructive" size="sm" className="h-10 gap-2 px-6 text-sm" disabled={killSwitchMutation.isPending} onClick={() => handleKillSwitchAction("ACTIVATE")}>
